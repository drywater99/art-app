...
RelArtworksThumbPage.propTypes = {
...
RelArtworksThumbPage.defaultProps = {
  title: 'No title defined',
  content: 'No content defined',
  bookmarked: false,
}

export default function RelArtworksThumbPage({
  title,
  image,
  id,
  onBookmark,
    ...
    <StyledLink to={`/artwork/${id}`}>
      <BorderCard>
        ...
        <ContentCard data-cy="card-content">
          <div>
            <h3>{relatedArtwork.date}</h3>
            <p>{relatedArtwork.title}</p>
            <small>{relatedArtwork.collecting_institution}</small>
          </div>
